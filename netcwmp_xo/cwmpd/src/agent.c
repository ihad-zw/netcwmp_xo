...
int cwmp_agent_init_parameter(cwmp_t * cwmp)
{
		 char name[1024] = {0};
	    char value[1024]= {0};
	...
	CWMP_SPRINTF_PARAMETER_NAME(name, 3, InternetGatewayDeviceModule, LANModule, IPAddressModuel);
	 local_ip   = cwmp_data_get_parameter_value(cwmp, cwmp->root, name,  cwmp->pool);
...
//
	...
	cwmp_log_debug("ConnectionRequestURL  =  %s\n",value);
...
void cwmp_agent_start_session(cwmp_t * cwmp)
{
    ...
    while (TRUE)
    {
        ...
        session->timeout = cwmp->http_timeout;
        ...
        while (!session_close)
        {
            ...
            case CWMP_ST_RECV:
                ...
                cwmp_log_debug("__%s___@%d__rv=%d\n",__FUNCTION__,__LINE__,rv);
...
static void print_param(parameter_node_t * param, int level)
{
  ...
  sprintf(fmt, "|%%-%ds%%s,  value:%%s", 0);
  

  cwmp_log_debug(fmt, "----", param->name, param->value);
  ...
  parameter_node_t * next = param->next_sibling;
...
int cwmp_agent_netlink_msg_cb(char *ifname,int etype)
{
	if(g_pstcwmp == NULL)
		return -1;

	cwmp_log_debug("netlink_msg_cb   ifrname = %s,etype = %d\n",ifname,etype);
	
	
	if(etype == NET_MSG_TYPE_ADD)
	{
		cwmp_event_set_value(g_pstcwmp, INFORM_BOOT, 1, NULL, 0, 0, 0);
		g_pstcwmp->new_request =  CWMP_TRUE;
...
void cwmp_agent_session(cwmp_t * cwmp)
{
	...
	char  envstr[] = "SOAP-ENV"; 
	char   encstr[] = "SOAP-ENC"; 

	g_pstcwmp = cwmp;
    cwmp_set_envelope_ns(envstr, encstr);

	netlink_init(cwmp_agent_netlink_msg_cb);
