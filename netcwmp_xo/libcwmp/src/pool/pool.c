...
#define POOL_ALIGNMENT sizeof(size_t)
#define pool_align(d, a)     (((d) + (a - 1)) & ~(a - 1))
#define pool_align_ptr(p, a)  ((void *)(((size_t ) (p) + ((size_t ) a - 1)) &( ~((size_t ) a - 1))))
...
void * pool_memalign(size_t alignment, size_t size)
{
	...
	{
		//        cwmp_log_error("posix_memalign() %uz bytes aligned to %uz failed",		size, alignment);
	}
	return p;
}
...
static void * pool_memalign(size_t alignment, size_t size)
{
	...
	if (p == NULL)
	{
		//        cwmp_log_error("memalign() %uz bytes aligned to %uz failed",		size, alignment);
	}
...
pool_t * pool_create(size_t size)
{
	...
	memset(p,0,size);
	p->d.last = (void*) p + sizeof(pool_t);
	p->d.last = pool_align_ptr(p->d.last,POOL_ALIGNMENT);
	p->d.end = (void *) p + size;
	p->d.next = NULL;
...
void pool_clear(pool_t *pool)
{
	...
	{
		n = p->d.next;
		//p->d.last = (void *)p + sizeof(pool_data_t);
		FREE(p);
		if (n == NULL)
		{
			break;
	...
	pool->d.last = (void *) pool + sizeof(pool_t);
	pool->d.next = NULL;
...
void * pool_palloc(pool_t *pool, size_t size)
{
	void  *m;
	pool_t  *p;
	size  = pool_align(size, POOL_ALIGNMENT);
	if (size <= pool->max)
	{
		...
		do
		{
			void * lastaddr =pool_align_ptr(p->d.last,POOL_ALIGNMENT);
			m = lastaddr;
			//m = p->d.last;
...
void * pool_pmemdup(pool_t * pool, const  void * ptr, size_t size)
{
	...
	void * p = pool_pcalloc(pool, size+1);
	if (!p)
	{
		return NULL;
	}
...
{
	size_t s;
	if (!ptr || strlen((char *)ptr)==0)
	{
		return NULL;
	}
	s = strlen((char *)ptr);
	
	return  (char *)pool_pmemdup(pool, ptr, s);
