...
{ 
	...
	cwmp_log_debug("0x%x 0x%x 0x%x, %ld\n",sock->sockdes,(sock->addr->sin4.sin_port),sizeof(struct sockaddr_in),(sock->addr->sin4.sin_addr.s_addr));
    if (connect(sock->sockdes, (const struct sockaddr *)&(sock->addr->sin4),
                sizeof(struct sockaddr_in)) == -1)
    {
		 	cwmp_log_debug("___%s____@%d___\n",__FUNCTION__,__LINE__);   
        return CWMP_ERROR;
    }   
...
int http_request_create(http_request_t ** request , pool_t * pool)
{
    ...
    req->dest = (http_dest_t*)pool_pcalloc(pool, sizeof(http_dest_t));
	
	
	cwmp_chunk_create(&req->readers, pool);
	cwmp_chunk_create(&req->writers, pool);
...
int http_read_body(http_socket_t * sock, int max)//, cwmp_chunk_t * body, pool_t * pool)
{
    ...
    while (bytes < max)
    {
        ...
        if ( (len = http_socket_read(sock, buffer, 1024)) < 0 )
        {
...
int http_read_request(http_socket_t * sock, http_request_t * request, pool_t * pool)
{
    ...
    len = cwmp_chunk_copy(hdata, header, 2047);
    cwmp_log_debug("http read request: %s\n", hdata);
    bytes += len;
    lines = http_split_headers(hdata, len, line);
    ...
    if(request->dest != NULL)
    {
		...
		TRstrncpy(request->dest->uri,uri,MAX_URI_LEN);
    }
	...
	request->cont_type=pool_pstrdup(pool,ctxtype);
	
	ctxlen = http_get_variable(parser, "Content-Length");
    cont_len = 0;
    if (ctxlen)
    {
        cont_len = TRatoi(ctxlen);
		request->cont_len = cont_len;
    }
...
int http_write_request(http_socket_t * sock , http_request_t * request, cwmp_chunk_t * chunk, pool_t * pool)
{
    ...
    len1 = TRsnprintf(buffer, HTTP_DEFAULT_LEN, header_fmt,
                    ...
                    dest->port,
                    "CPE Netcwmp Agent",
					(request->cont_type == NULL ? "text/xml; charset=utf-8" : request->cont_type),
                    len2);
    if(len2 > 0)
    {
