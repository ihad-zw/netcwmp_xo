...
int httpd_cwmp_proc(cwmp_t *cwmp,http_socket_t * sock, http_request_t * request, pool_t * pool)
{
	...
	if(parser != NULL)
	{
		ctxtype = http_get_variable(parser, "Content-Type");
		if(ctxtype != NULL)
		{
			cwmp_log_info("content-type :: %s\n",ctxtype);
			if(TRstrcasecmp(ctxtype,CONTENT_TYPE_JSON)==0)
			{
				http_json_proc(cwmp,sock,request,pool);
				
			}
			else if(TRstrcasecmp(ctxtype,CONTENT_TYPE_URLENCODED)==0)
			{
				http_urlcmd_proc(cwmp,sock,request,pool);
			}
			else
			{
				
				cwmp_log_info("Content-Type  not support\na");
			}
		}
	}
...
static size_t httpd_request_write_callback(char *data, size_t size, size_t nmemb, void * calldata)
{
	http_request_t * request = (http_request_t *)calldata;
	

    cwmp_chunk_write_string(request->readers, data, size * nmemb, request->envpool);
...
int httpd_build_server(cwmp_t * cwmp)
{
    ...
    if(pool == NULL)
    {
	  cwmp_log_error("__ %s__ pool == NULL\n",__FUNCTION__);
        exit(-1);
    }
    ...
    while (1)
    {
        ...
        for (i=0; (i<MAX_CLIENT_NUMS) && (nready > 0) ; i++)
        {
            ...
            if ((fd != -1) && FD_ISSET(fd, &rdset))
            {
				...
				http_socket_set_writefunction(s, httpd_request_write_callback, request);
		  	 ...
		  	 if (request->dest!=NULL && TRstrcasecmp("/", request->dest->uri) != 0)
			{
					cwmp_log_debug("__%s__@%d__uri = %s\n",__FUNCTION__,__LINE__,request->dest->uri);
					if(TRstrcasecmp(HTTPD_CWMP, request->dest->uri) == 0)
					{
						httpd_cwmp_proc(cwmp,s, request, http_socket_get_pool(s));
					}
			...
			else
			{
	                ...
	                if (cwmp->cpe_auth)
	                {
	                    ...
	                    cwmp_conf_get("MSERVER:ConnectionRequestUsername", cpe_user);
	                    cwmp_conf_get("MSERVER:ConnectionRequestPassword", cpe_pwd);
