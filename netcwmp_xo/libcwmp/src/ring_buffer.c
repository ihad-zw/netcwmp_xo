...
uint32_t __ring_buffer_get(struct ring_buffer *ring_buf, void * buffer, uint32_t size)
{
    ...
    uint32_t len = 0;
     ...
     if( ring_buf->in >=  ring_buf->out)
    {
    	if((ring_buf->in- ring_buf->out) < size )
    		return 0;   
    }
    else
    {
    	if((ring_buf->size-ring_buf->out+ring_buf->in)<size)
    		return 0;
    }
...
//    ring_buf->out += size;
    ring_buf->out =(ring_buf->out + size)%ring_buf->size;
    return size;
}
//向缓冲区中存放数据
uint32_t __ring_buffer_put(struct ring_buffer *ring_buf, void *buffer, uint32_t size)
{
    ...
    if( ring_buf->in >=  ring_buf->out)
    {
    	if((ring_buf->size-ring_buf->in + ring_buf->out) < size )
    		return 0;   
    }
    else
    {
    	if((ring_buf->out-ring_buf->in)<size)
    		return 0;
    }
    ...
    ring_buf->in = (ring_buf->in + size)%ring_buf->size;
