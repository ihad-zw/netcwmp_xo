...
//	TYPE_BASE64,	//base
    ...
    if(! TRstrncasecmp(type, "Int",3))
    {
        return TYPE_INT;
    }
    else if(! TRstrncasecmp(type, "unsignedInt",11))
    {
        return TYPE_UNSIGNEDINT;
    }
    else if(! TRstrcasecmp(type, "String"))
    {
        return TYPE_STRING;
    }
    else if(! TRstrcasecmp(type, "String(16)"))
    {
        return TYPE_STRING16;
    }
    else if(! TRstrcasecmp(type, "String(32)"))
    {
        return TYPE_STRING32;
    }
    else if(! TRstrcasecmp(type, "String(64)"))
    {
        return TYPE_STRING64;
    }
    else if(! TRstrcasecmp(type, "String(128)"))
    {
        return TYPE_STRING64;
    }
    else if(! TRstrcasecmp(type, "String(256)"))
    {
        return TYPE_STRING64;
    }
    else if(! TRstrcasecmp(type, "String(1024)"))
    {
        return TYPE_STRING64;
    }
    else if(! TRstrcasecmp(type, "DateTime"))
    {
        return TYPE_STRING64;
    }
    else if(! TRstrcasecmp(type, "Boolean"))
    {
        return TYPE_STRING64;
    }
     else if(! TRstrncasecmp(type, "base",4))
    {
        return TYPE_BASE64;
    }
...
parameter_node_t * cwmp_get_parameter_node(parameter_node_t * root, const char * param_name)
{
    ...
    {
	  nodetmp = node;
        while (nodetmp && nodetmp->name)
        {
            if (TRstrcmp(nodetmp->name, name) == 0)
            {
...
int cwmp_get_parameter_node_value(cwmp_t * cwmp, parameter_node_t * node, const char * name, char ** value, pool_t * pool)
{
    ...
    if (node->get)
    {
        return (*node->get)(cwmp,node, name, value, pool);
    }
    else
    {
        return FAULT_CODE_9000;
...
int cwmp_set_parameter_node_value(cwmp_t * cwmp, parameter_node_t * node, const char * name, const char * value, int value_length)
{
    ...
    if (node->set)
    {
        return (*node->set)(cwmp, node,name,  value, value_length, callback_register_task);
    }
    else
    {
        if (node->value)
    ...
    while (parameterNode)
    {
        ...
        else
        {
            ...
            if (pn->get)
            {
            	//exec get
            	char * value = NULL;
		rc = (*pn->get)(env->cwmp,pn, name, &value, pool);
		if(rc != FAULT_CODE_OK)
		{
		   fault->fault_code = rc;
		   return CWMP_ERROR;
    ...
    *ppl = cwmp_create_parameter_list(env);
	...
	if (!parameter)
        ...
        else
        {
            ...
            {
		//exec set function
		parameter->fault_code =  (*pn->set)(env->cwmp, pn,name,  value, TRstrlen(value), callback_register_task);
            }
	    else
	    {
...
int cwmp_parse_upload_message(env_t * env , xmldoc_t *doc, upload_arg_t ** pularg, fault_code_t *fault)
{
    ...
    upload_arg_t * ularg = pool_pstrdup(env->pool, sizeof(download_arg_t));
